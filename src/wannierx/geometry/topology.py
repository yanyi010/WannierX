...
def chern_number(
    ...
    Parameters:
        ...
        occupied: band indices forming the subspace (e.g. range(n_occ)).
        isolation_atol, isolation_rtol: tolerances for checking that the
            occupied/unoccupied gap never closes on the mesh and that the
            occupied subspace is internally well defined.

    Returns:
        Chern number as a float; converges to an integer.

    Failure behavior: raises :class:`DegeneracyError` when the occupied
    subspace is not isolated (gap to the rest closes on the mesh) or when
    the occupied subspace itself is degenerate in a way that makes the
    band selection ambiguous. No regularization is applied.
    """
    ...
    lower_gap = (
        np.min(E_sorted[:, lo] - E_sorted[:, lo - 1]) if lo > 0 else np.inf
    )
    upper_gap = (
        np.min(E_sorted[:, hi + 1] - E_sorted[:, hi]) if hi < n_orb - 1 else np.inf
    )
    gap_floor = min(lower_gap, upper_gap)
    scale = max(float(np.max(np.abs(E_sorted))), 1.0)
    if not np.isfinite(gap_floor) or gap_floor <= isolation_atol + isolation_rtol * scale:
        raise DegeneracyError(
            f"occupied subspace is not isolated on the mesh (min boundary gap "
            f"{gap_floor}); refine the mesh or choose an isolated subspace"
        )
